...
export async function POST(request: NextRequest) {
  ...
  if (event === "installation" || event === "installation_repositories") {
    ...
    if (action === "created") {
      ...
      if (installId && senderId) {
        try {
          await kv.set(`installation:${installId}:userId`, senderId);
          console.log(`[DocuPilot] Mapped installation ${installId} -> user ${senderId} (${sender})`);
        } catch (err) {
          console.error("[DocuPilot] Failed to save installation mapping:", err);
        }
      }
...
async function processDocGeneration(
  ...
  // Try to get user's OAuth token (with repo scope) for write operations
  let writeToken = installationToken;
  try {
    const userId = await kv.get<number>(`installation:${installationId}:userId`);
    if (userId) {
      const userToken = await kv.get<string>(`user:${userId}:token`);
      if (userToken) {
        writeToken = userToken;
        console.log(`[DocuPilot] Using user OAuth token for writes (userId: ${userId})`);
      } else {
        console.log(`[DocuPilot] No OAuth token found for userId ${userId}, using installation token`);
      }
    } else {
      console.log(`[DocuPilot] No user mapping for installation ${installationId}, using installation token`);
    }
  } catch (err) {
    console.error("[DocuPilot] Failed to lookup user token, using installation token:", err);
  }

  // Use installationToken for reads (always has read access), writeToken for writes
  const token = installationToken;

  console.log(`[DocuPilot] Generating docs for ${repoFullName}@${branch} (installation: ${installationId})`);
  console.log(`[DocuPilot] Read token length: ${token.length}, Write token length: ${writeToken.length}`);
  ...
  // 4. Check for existing open DocuPilot PR
  const existingPR = await findOpenDocuPilotPR(token, owner, repo, branch);

  let branchName: string;
  let prUrl: string;

  if (existingPR) {
    // Update existing PR branch instead of creating a new one
    branchName = existingPR.head_ref;
    console.log(`[DocuPilot] Found existing PR #${existingPR.number}, updating branch ${branchName}`);

    // Update files on the existing branch
    if (docs.readme) {
      const readmeSha = await getFileSha(writeToken, owner, repo, "README.md", branchName);
      await createOrUpdateFile(
        writeToken, owner, repo, "README.md", docs.readme,
        "docs: update README.md via DocuPilot", branchName, readmeSha
      );
    }
    if (docs.changelog) {
      const changelogSha = await getFileSha(writeToken, owner, repo, "CHANGELOG.md", branchName);
      await createOrUpdateFile(
        writeToken, owner, repo, "CHANGELOG.md", docs.changelog,
        "docs: update CHANGELOG.md via DocuPilot", branchName, changelogSha
      );
    }

    // Notify on the existing PR
    const updatedFiles = [
      ...(docs.readme ? ["README.md"] : []),
      ...(docs.changelog ? ["CHANGELOG.md"] : []),
    ];
    await addPRComment(
      writeToken, owner, repo, existingPR.number,
      `🔄 Documentation updated based on latest push to \`${branch}\`.\n\nUpdated: ${updatedFiles.join(", ")}`
    );
    prUrl = existingPR.html_url;
    console.log(`[DocuPilot] Updated existing PR: ${prUrl}`);
  } else {
    // Create new branch and PR
    branchName = `docupilot/update-docs-${Date.now()}`;
    const baseSha = await getBranchSha(token, owner, repo, branch);
    await createBranch(writeToken, owner, repo, branchName, baseSha);
    ...
    const pr = await createPullRequest(
      writeToken, owner, repo,
      "📝 DocuPilot: Update documentation",
      "This PR was automatically generated by [DocuPilot](https://docupilot-alpha.vercel.app) — AI-powered documentation that stays in sync with your code.\n\nDocuPilot analyzed your latest code changes and updated the documentation accordingly.\n\n---\n\n🤖 *Free for 1 repo — [Get DocuPilot for your projects](https://docupilot-alpha.vercel.app?utm_source=github_pr&utm_medium=referral&utm_campaign=viral)*",
      branchName, branch
    );
    prUrl = pr.html_url;
    console.log(`[DocuPilot] New PR created: ${prUrl}`);
  }
